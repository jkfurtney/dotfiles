...
(defadvice slime-compilation-finished (after jkf/show-ecl-defun-c-code (result))
  "open the intermediate C file generated by ECL when compiling a defun."
  (find-file (jkf--find-last-modified-temporary-c-file)))
;(ad-activate 'slime-compilation-finished)

;; (defun jkf/ecl-show-defun-c-code ()
;;   (interactive)
;;   (apply #'jkf/ecl-show-defun-c-code-region
;;          (slime-region-for-defun-at-point)))
;; (defun jkf/ecl-show-defun-c-code-region (start end)
;;   (interactive "r")
;;   (slime-flash-region start end)
;;   (let* ((tmp-file (make-temp-file "ecl")))
;;     (message tmp-file)
;;     (write-region start end tmp-file)
;;     (slime-eval-async
;;         `(compile-file ,tmp-file :c-file t)
;;       ; we can add a function pointer here to be called at the end of the
;;       ; async call
;;       )
;;     (find-file (format "%s.c" tmp-file))))
; hack to open intermediate c file from ecl compilation:
; sort this list
;; (map 'list (lambda (fname) (fifth (file-attributes fname)))(directory-files temporary-file-directory t ".c$"))
;; (fifth (file-attributes (first (directory-files temporary-file-directory t ".c$"))))
    ...
    (if (> a-0 b-0)
      ...
      (if (> a-1 b-1)
        ...
        (if (> a-2 b-2)
            t)
        nil))))
;; (first (sort (directory-files temporary-file-directory t "\\.c$") 'jkf--sort-file-modification-times))

;; (fifth (file-attributes (first (sort (directory-files temporary-file-directory t "\\.c$") 'jkf--sort-file-modification-times))))
...
(defun get-last () (interactive) (find-file (jkf--find-last-modified-temporary-c-file)))
